...
class VisualCoreLanguageConditioned(EncoderCore, BaseNets.ConvBase):
    """
    A network block that combines a visual backbone network with optional pooling
    and linear layers. It includes a language embedding input in forward to be passed to the backbone
    """
    def __init__(
        self,
        input_shape,
        ...
        Args:
            ...
            feature_dimension (int): if not None, add a Linear layer to
                project output into a desired feature dimension
        """
        super(VisualCoreLanguageConditioned, self).__init__(input_shape=input_shape)
        self.flatten = flatten
